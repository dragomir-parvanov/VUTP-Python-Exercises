...
for contact in parsedJson:
    def _json_object_hook(d): return Contact('X', d.keys())(*d.values())
    def json2obj(data): return json.loads(data, object_hook=_json_object_hook)
    contact = json.loads(contact)
    bindedModel = Contact(contact["name"],contact["address"],contact["birthDay"],contact["phoneNumber"],contact["email"],contact["profession"],contact["interests"])
...
def createContact():
        ...
        contact = Contact(name=nameInput,address=addressInput,birthDay=birthDayInput,phoneNumber=phoneNumberInput,email=emailInput,profession=professionInput,interests=interestsInput)
        return contact
class Commands:
...
def deleteFromContacts(name):
    global allContacts
    allContacts = filter(lambda x: name == str(name), allContacts)
